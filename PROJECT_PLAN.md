...
| `check_summary(week, summary)` | 주차명, 요약문 | 요약 숫자가 집계와 일치하는지 검사 (Phase 7) |
...
**동기**: Phase 6 이후 테스트에서 허점이 드러났다. 요약은 자유 텍스트이므로
틀린 숫자를 넣어도 아무것도 막지 않았다.

```
실제 집계:  마감 6건 / 완료 5건 / 83.3%
주입한 요약: "금주 마감 20건 중 19건 완료(95.0%)로 목표를 초과 달성했다."
-> 그대로 파일에 기록됨
```

같은 문서 안에서 요약과 지표표가 모순되는 상태로 생성된다.
"숫자는 코드가 만든다"는 이 프로젝트의 원칙이 요약 문장에는 관철되지 않았다.
Phase 5의 프롬프트 규칙은 지시일 뿐 강제가 아니었다.

**구현**

- `allowed_numbers(week)` — 요약에 등장해도 되는 숫자와 그 출처를 만든다
- `check_summary(week, summary)` — 집계에 없는 숫자를 찾아낸다
- `build(..., strict=True)` (기본) — 불일치면 `ValueError`로 저장을 거부하고,
  사용 가능한 값의 출처를 에러 메시지에 담는다. 모델은 이를 보고 재호출한다
- MCP 툴 `check_summary` 추가 (총 7개). 저장 전 미리 확인용
- `build_report` 반환에 `summary_numbers_verified` 추가 — 통과 근거

`report_builder`는 MCP 비의존을 유지하려고 `ValueError`를 던지고,
서버의 `_guard`가 `ToolError`로 변환한다.

**허용 범위 설계**

집계에서 유도되는 값만 허용한다. 오탐을 줄이기 위해 다음도 포함했다.

- 직전 주차의 완료율과 증감폭(%p) — 커맨드가 추세 비교를 지시하므로 필요하다
- 주차 번호·연도·주 기간의 월/일
- 1..주차수 — `"3주 연속 하락"` 같은 표현용

**전체 건수 기준 `completion_rate`는 의도적으로 제외했다.**
요약에 그 값을 쓰는 것 자체가 보고 규칙 위반이므로 검증에서 걸리는 게 옳다.
결과적으로 이 검증이 지표 선택 규칙까지 강제한다.

**우회 경로**: `strict=False`. 사람이 판단해 넘겨야 하는 경우를 위해 남겼다.
MCP 툴로는 노출하지 않았다.
...
| 항목 | 결과 |
|---|---|
| 집계에 없는 숫자(20/19/95) 거부 | 거부 |
| 왜곡 지표(45.5%) 거부 | 거부 |
| 정상 요약 통과 + 숫자 출처 반환 | 4개 숫자 확인 |
| 직전 주 값(83.3%)·증감폭(11.9%p) 허용 | 통과 |
| 주차 수 표현("3주 연속") 허용 | 통과 |
| `strict=False` 우회 | 동작 |
| 빈 요약은 검증 생략 (초안 경로) | 동작 |

`verify_all.py`에 `6. 요약 검증` 그룹으로 편입. **전체 55/55 통과.**
